...
plt.plot( np.arange(len(y_test_true)), y_test_true, label='Test True',color='#D614AC')##EC1A23
# plt.plot(len(y_train_true) + np.arange(len(y_test_true)), y_test_pred, label='Test Predicted',color='darkviolet')
plt.plot( np.arange(len(y_test_true)), y_test_pred, label='Test Predicted',color='#54966D')#deepskyblue royalblue  green#5BB847
plt.legend()
...
# plt.plot(losses, label='Training Loss',color='#EC1A23')#比亚迪#2DA32D  中国联通 #EC1A23 平安银行#3E848C
# plt.title('China Unicom Loss Over Epochs') #China Unicom
# plt.xlabel('Epoch')
# plt.ylabel('Loss')
# plt.legend()
# plt.grid(True)
# plt.savefig('./中国联通Loss.jpg',dpi=1000)
# plt.show()

#专利
plt.plot(losses, label='训练时损失值',color='#EC1A23')#比亚迪#2DA32D  中国联通 #EC1A23 平安银行#3E848C
plt.title('(c).C股票损失值随训练批次变化图') #China Unicom
plt.xlabel('训练批次')
plt.ylabel('损失值')
plt.legend()
plt.grid(True)
plt.savefig('./C股票Loss专利.jpg',dpi=1000)
plt.show()