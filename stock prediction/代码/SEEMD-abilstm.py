...
class BiLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        # LSTM output: [batch_size, seq_len, 2 * hidden_size]
        lstm_out, _ = self.lstm(x)
        # Attention output and attention weights
        attention_out, _ = self.attention(lstm_out)
        # Pass the attention output to the fully connected layer
...
plt.plot(np.arange(len(y_test_true)), y_test_true, label='Test True',color='#EC1A23')#平安银行 #2401F7 比亚迪#3E848C  中国联通#908D01   上证#EC1A23   综指##DDD305   沪深#508AB2
...
plt.plot(np.arange(len(y_test_true)), y_test_pred, label='Test Predicted',color='#00A651')#deepskyblue royalblue 平安银行 #F20505 比亚迪#F29F05 中国联通#1A83B2 上证#00A651 综指##03CDA7 沪深#B36A6F
